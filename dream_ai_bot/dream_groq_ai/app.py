...
Notes
RU/EN file paths are exactly kb/parsed_faqs_ru.json and kb/parsed_faqs_en.json. If you ever rename them, set GH_JSON_RU / GH_JSON_EN env vars to the new paths.

If GitHub answers with an empty list or the wrong shape, the server returns an error instead of silently showing 0 items. This makes misconfigurations obvious.

No local cache, no background polling, no fallback reads. Admin edits write directly to the same GH paths and update immediately.
...
can you make the script full utf8. do not omit anything 

ChatGPT said:

Thought for 10s
Here’s a full, drop-in app.py that’s the same GitHub-only build you asked for, but 100% ASCII/UTF-8-safe (no smart quotes or em-dashes). It also includes an explicit UTF-8 coding header at the top.

python
Copy code
# -*- coding: utf-8 -*-
# app.py - GitHub-only FAQs + Groq RAG (ASCII-safe source)
...
# ===== Groq bot =====
from groq_bot import warm_index, groq_answer, reload_index
from faq_parser import load_faqs  # only for /api/rag-status if local caches exist
...
LOCAL_JSON_EN  = str(KB_DIR / "parsed_faqs_en.json")  # not used for reads; status only
LOCAL_JSON_RU  = str(KB_DIR / "parsed_faqs_ru.json")  # not used for reads; status only
...
# =====================================================================================
# GitHub kb-data (SOURCE OF TRUTH)
# =====================================================================================
GH_OWNER   = os.getenv("GH_OWNER",  "dream-framework")
GH_REPO    = os.getenv("GH_REPO",   "dream")
GH_BRANCH  = os.getenv("GH_BRANCH", "kb-data")
...
def _normalize_faq_json(raw_bytes: bytes) -> Dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"Invalid JSON in GitHub file: {e}") from e

    # Accept shapes: {"faqs":[...]}, {"items":[...]}, or top-level list
    faqs = []
    if isinstance(obj, dict):
        if isinstance(obj.get("faqs"), list):
            faqs = obj["faqs"]
    ...
    for f in faqs:
        ...
        out.append({"number": n, "question": q, "answer": a})
    out.sort(key=lambda x: x["number"])
    return {"metadata": (obj.get("metadata") if isinstance(obj, dict) else {}), "faqs": out}
...
# =====================================================================================
# Admin (GET UI only; CRUD uses GH /faq API)
# =====================================================================================
ADMIN_TOKEN = os.getenv("ADMIN_TOKEN", "January@1")
...
def chat():
    ...
    if best and score > 0:
        return jsonify({"reply": f"**{best['question']}**\n\n{best['answer']}", "source": f"faq-{lang}"})
    return jsonify({"reply": "I could not find that in the FAQs. Try the FAQ Bot or rephrase your question."})

# =====================================================================================
# Status (only reports counts if local files exist)
# =====================================================================================
@app.get("/api/rag-status")
def rag_status():
    def count(local_path):
